...
export type VehicleType = "Car" | "Motorbike" | "Truck" | "SUV" | "Van" | "Other"
...
export function MarketplaceProvider({ children }: { children: React.ReactNode }) {
  ...
  const createListing = async (
    listingData: Omit<VehicleListing, "id" | "createdAt" | "updatedAt" | "status" | "views" | "favorites">, 
    images: File[]
  ): Promise<string> => {
    if (!user) {
    ...
    try {
      ...
      // Add to Firestore
      const listingRef = await addDoc(collection(db, "vehicleListings"), listingWithMeta)
      ...
      if (images.length > 0) {
        ...
        await updateDoc(
          doc(db, "vehicleListings", listingId),
          { 
            images: imageUrls, 
  ...
  const updateListing = async (
    ...
    try {
      ...
      if (newImages && newImages.length > 0) {
        const newImageUrls = await uploadImages(newImages, `listings/${listingId}/additions`)
  ...
  const deleteListing = async (listingId: string, isAdminOverride: boolean = false): Promise<void> => {
    ...
    try {
      ...
      toast({
        ...
        description: "Your vehicle listing has been removed from the marketplace."
      })
      ...
      toast({
        ...
        description: error.message || "There was a problem deleting your listing",
        variant: "destructive"
      })
      throw error
    } finally {
    ...
    try {
      ...
      if (filters) {
        ...
        if (filters.vehicleType && filters.vehicleType.length > 0) {
          queryConstraints.push(where("vehicleType", "in", filters.vehicleType))
        }
  ...
  const exportListings = async (format: 'csv' | 'json' = 'csv') => {
    try {
      ...
      if (format === 'csv') {
        const headers = ['id', 'title', 'make', 'model', 'year', 'price', 'status', 'createdAt'] as (keyof VehicleListing)[]
        const csvContent = [
          headers.join(','),
        ...
        link.download = `vehicle-listings-${new Date().toISOString().split('T')[0]}.csv`
        link.click()
      } else {
        ...
        const link = document.createElement('a')
        link.href = URL.createObjectURL(blob)
        link.download = `vehicle-listings-${new Date().toISOString().split('T')[0]}.json`
        link.click()
      }
    } catch (error) {
