  ...
  useEffect(() => {
    ...
    const fetchSimilarVehicles = async () => {
      try {
        ...
        let candidates = result.listings.filter(
        ...
        if (candidates.length < 3 && make) {
          ...
          if (process.env.NEXT_PUBLIC_DEBUG_SIMILAR === "true") {
            // eslint-disable-next-line no-console
            console.log(
              "[SimilarVehicles] Fetch 2 relaxed filters",
              relaxedFilters
            );
          }
          const more = await getListings(relaxedFilters, 9);
          const additional = more.listings.filter(
            (v) =>
        ...
        if (candidates.length < 3) {
          ...
          if (process.env.NEXT_PUBLIC_DEBUG_SIMILAR === "true") {
            // eslint-disable-next-line no-console
            console.log(
              "[SimilarVehicles] Fetch 3 global fallback",
              globalFallbackFilters
            );
          }
          const global = await getListings(globalFallbackFilters, 12);
          const globalExtra = global.listings.filter(
            (v) =>
              v.id !== currentVehicleId &&
              !candidates.find((c) => c.id === v.id)
          );
  ...
  const Header = (
    <div className="text-center">
      ...
      <p className="text-gray-400">
        People who viewed this vehicle also consider
      </p>
    </div>
  );
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="text-center text-sm text-muted-foreground border border-dashed border-border rounded-lg p-6">
          {error
            ? `Unable to load similar vehicles: ${error}`
            : "No similar vehicles available yet. Check back soon."}
        </div>
      </div>
    );
  }
          ...
          <Link key={vehicle.id} href={`/vehicles/${vehicle.id}`}>
            <Card className="h-full hover:bg-gray-800/50 transition-colors">
              <div className="relative aspect-[4/3]">
                ...
                <Image
                  src={
                    vehicle.mainImage || vehicle.images[0] || "/placeholder.svg"
                  }
                  alt={vehicle.title}
                  fill
              ...
              <CardContent className="p-4">
                ...
                <p className="text-sm text-gray-400 line-clamp-2">
                  {vehicle.description ||
                    `${vehicle.year} ${vehicle.make} ${vehicle.model} in ${vehicle.condition} condition.`}
                </p>
              </CardContent>
                ...
                <div className="flex items-center justify-between w-full">
                  <div className="text-lg font-bold">
                    {(vehicle as any).directImport === true
                      ? // Preserve raw $ string if stored, otherwise format as USD
                        typeof vehicle.price === "string" &&
                        vehicle.price.trim().startsWith("$")
                        ? vehicle.price
                        : new Intl.NumberFormat("en-US", {
                            ...
                            parseFloat(
                              String(vehicle.price).replace(/[^0-9.-]+/g, "")
                            ) || 0
                          )
                      : `KES ${parseInt(
                          String(vehicle.price || "0").replace(
                            /[^0-9.-]+/g,
                            ""
                          ) || "0"
                        ).toLocaleString()}`}
                  ...
                  <Badge variant="outline">
                    {vehicle.negotiable ? "NEGOTIABLE" : "FIXED PRICE"}
                  </Badge>
                </div>
              </CardFooter>
            </Card>
